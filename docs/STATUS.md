...
| Crate | State | Notes |
|---|---|---|
| `creda-events` | ✅ | Event model, 10 event types, canonical CBOR, Blake3, UUIDv7, algorithm-agile signatures. Tombstoned-husk reduction (§3.4.6): scrubs `Assert`/`Amend` demographics to empty and voids the content hash, keeping the envelope. |
...
- **Spec §13 Open Questions** — the canonical list (DHT privacy, revocation bounds 2/3, storage
  substrate, R4→R5, FAST Consent F1–F5, etc.). Match/confidence calibration (§5.3.2) is now **resolved
  as a methodology** (`docs/matching-calibration.md`); only the per-deployment calibrated numbers (which
  need real data) remain.
