...
### 13.9 Cross-Border and Federation

#### 13.9.1 Patient Mobility Across National Networks

**Reference:** Sections 1 (Overview), 6 (Network Architecture), 9.2 (Tokenization), 12 (Migration and Adoption Path)

**The question:** Credara is scoped to US healthcare and assumes a single national trust framework, a single network salt epoch, and US-shaped identifiers. What happens when a patient receives care outside that network — travels abroad, or is treated in a jurisdiction running its own Credara-like deployment? Four sub-problems compound. (1) **The admission gate is jurisdictional:** the Network Participation Agreement is a multi-party HIPAA Business Associate Agreement, which a non-US institution that is not a HIPAA covered entity cannot sign — so it cannot join the network, and the traveling patient's foreign encounters are simply invisible. (2) **Identity matching loses its strongest signals:** SSN fragment, US MRNs, payer member IDs, and USPS/ZIP+4-normalized addresses are absent abroad, collapsing matching onto the weakest fields (family name, DOB, sex — the same fields the DHT routing key derives from in §6.1.6) and onto transliteration cases the §9.2.3 normalization rules do not address. (3) **The network salt isolates networks by construction:** tokens are deterministic only within a salt epoch (§9.2.1–9.2.2), so the same patient's tokens do not match across two independently-salted networks without an explicit, privacy-sensitive bridging step. (4) **Data-residency and divergent legal regimes conflict with the replication model:** GDPR (including the Article 17 erasure right) versus HIPAA, plus data-localization statutes, are at odds with a gossip network that replicates events — including tokenized demographics, which remain personal data — across peers in multiple jurisdictions.

**Why it's open:** v1 deliberately scopes to a single US national network, where the absence of a national patient identifier makes the cross-institutional problem most acute and the regulatory surface (HIPAA, the Cures Act information-blocking rule, TEFCA, 42 CFR Part 2) is singular. Cross-border operation is not a v1 requirement, and solving it inside the existing single-network architecture would violate data-residency law and weaken the matching guarantees the system exists to provide. The realistic shape is **federation** — independent national or regional networks, each with its own trust framework, salt, and legal basis, bridged at well-defined gateways (mirroring how TEFCA federates QHINs domestically and how the EU's eHDSI / MyHealth@EU links national contact points for cross-border summaries) — rather than one global graph. The clinical hand-off surface for a traveling patient is an existing standard, the HL7 FHIR **International Patient Summary** (IPS; ISO 27269), not an extension of Credara's identity graph. Notably, the portable-authorization primitive generalizes across borders more readily than identity: a signed, detachable `AuthorizationGrant` is verifiable offline given a shared trust anchor for the issuer's keys, whereas cross-network identity linking requires solving salt-bridging and identifier divergence first.

**Closure condition:** A federation design document, developed when a second national or regional network exists or a founding institution requires cross-border continuity. It should specify: the inter-network trust-anchor and admission model (how one network's legal coordinator recognizes another's, replacing the HIPAA-BAA-based NPA at network boundaries); a cross-salt token-bridging protocol carrying an explicit cryptographer review at the same bar as the §9.5 / §13.3 DHT-privacy work (do not hand-roll); jurisdiction-aware replication and data-residency scoping so events do not cross borders unlawfully; and the mapping between Credara identity and consent artifacts and the HL7 FHIR IPS for clinical hand-off. Engage the Sequoia Project (TEFCA), the EU eHDSI / MyHealth@EU technical bodies, and the HL7 IPS workgroup.

### 13.10 Tracking and Closure
