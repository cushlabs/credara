# Code of Conduct

The Credara project adopts the **Contributor Covenant, version 2.1** as its code of
conduct. The full text is published at:

- https://www.contributor-covenant.org/version/2/1/code_of_conduct/

A copy in Markdown is available at the Contributor Covenant repository:

- https://github.com/EthicalSource/contributor_covenant

By participating in this project — contributing code, opening issues, reviewing pull
requests, or taking part in any project space — you agree to uphold that standard.

## Scope

This Code of Conduct applies within all project spaces and also applies when an
individual is officially representing the project in public spaces.

## Enforcement

Concerns may be reported privately to the project maintainers. Reports will be reviewed
and handled in accordance with the Enforcement Guidelines described in the Contributor
Covenant.

> _Maintainer note: replace this line with the project's monitored conduct contact
> address before public launch._
